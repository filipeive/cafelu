...
class ReportController extends Controller
{
    public function index(Request $request)
    {
        ...
        return view('reports.index', array_merge(
            ...
            [
                ...
                'profitChartData' => $salesChart['profitData'],
                'paymentMethodLabels' => $paymentMethod['labels'],
                'paymentMethodData' => $paymentMethod['data'],
                'paymentMethodAmountData' => $paymentMethod['amountData'],
    ...
    public function salesByProduct(Request $request)
    {
        ...
        $sales = Product::select('products.name', 'products.purchase_price', 'products.selling_price')
            ->join('sale_items', 'products.id', '=', 'sale_items.product_id')
            ->join('sales', 'sales.id', '=', 'sale_items.sale_id')
            ->whereBetween('sales.sale_date', [$dateFrom, $dateTo])
            ->groupBy('products.id', 'products.name', 'products.purchase_price', 'products.selling_price')
            ->selectRaw('SUM(sale_items.quantity) as quantity_sold, SUM(sale_items.quantity * sale_items.unit_price) as total_revenue')
    ...
    public function salesByCategory(Request $request)
    {
        ...
        return view('reports.sales_by_category', compact('sales', 'dateFrom', 'dateTo'));
    }
    ...
    public function salesByPaymentMethod(Request $request)
    {
        ...
        $sales = Sale::whereBetween('sale_date', [$dateFrom, $dateTo])
            ...
            ->selectRaw('COUNT(*) as sales_count, SUM(total_amount) as total_revenue')
            ->orderByDesc('total_revenue')
            ->get();

        return view('reports.sales_by_payment_method', compact('sales', 'dateFrom', 'dateTo'));
    }
    ...
    public function salesByDate(Request $request)
    {
        ...
        $sales = Sale::whereBetween('sale_date', [$dateFrom, $dateTo])
            ...
            ->selectRaw('DATE(sale_date) as date, COUNT(*) as sales_count, SUM(total_amount) as total_revenue')
            ->orderBy('date')
            ->get();

        return view('reports.sales_by_date', compact('sales', 'dateFrom', 'dateTo'));
    }
    ...
    private function getDadosDetalhados($reportType, $dateFrom, $dateTo)
    {
        ...
        if ($reportType === 'sales' || $reportType === 'all') {
            $dados['sales'] = Sale::with(['user', 'saleItems.product'])
                ->whereBetween('sale_date', [$dateFrom, $dateTo])
                ->select([
                    'id',
                    'sale_date',
        ...
        if ($reportType === 'expenses' || $reportType === 'all') {
            $dados['expenses'] = Expense::with(['user'])
                ...
                ->map(function ($product) {
                    ...
                    $salesData = DB::table('sale_items')
                        ...
                        ->select([
                            ...
                            DB::raw('SUM(sale_items.quantity * sale_items.unit_price) as revenue_generated')
                        ])
                        ->first();
    ...
    public function profitLoss(Request $request)
    {
        ...
        $productProfitability = Product::select('products.name')
            ...
            ->selectRaw('
                ...
                SUM((sale_items.quantity * sale_items.unit_price) - (sale_items.quantity * products.purchase_price)) as profit
            ')
            ->orderByDesc('profit')
            ->get();
    ...
    public function cashFlow(Request $request)
    {
        ...
        $cashOutflows = Expense::whereBetween('expense_date', [$dateFrom, $dateTo])
            ->get()
            ->groupBy('category')
            ->map(function ($expenses) {
                return $expenses->sum('amount');
    ...
    public function customerProfitability(Request $request)
    {
        ...
        $customerAnalysis = Sale::with(['saleItems.product'])
            ->whereBetween('sale_date', [$dateFrom, $dateTo])
            ...
            ->map(function ($sales, $customerName) {
                ...
                $totalCost = $sales->flatMap->saleItems->sum(function ($item) {
                    return $item->quantity * ($item->product->purchase_price ?? 0);
                });
    ...
    public function abcAnalysis(Request $request)
    {
        ...
        $productAnalysis = Product::join('sale_items', 'products.id', '=', 'sale_items.product_id')
            ->join('sales', 'sale_items.sale_id', '=', 'sales.id')
            ->whereBetween('sales.sale_date', [$dateFrom, $dateTo])
    ...
    public function salesReport(Request $request)
    {
        ...
        $topProducts = $sales->flatMap->saleItems->groupBy('product.name')->map(function ($group, $productName) {
            ...
            return [
                ...
                'profit' => $group->sum(function ($item) {
                    return $item->quantity * $item->unit_price;
                }) - $group->sum(function ($item) use ($product) {
                    return $item->quantity * ($product->purchase_price ?? 0);
                })
            ];
    ...
    public function expensesReport(Request $request)
    {
        ...
        $expensesByCategory = $expenses->groupBy('category')->map(function ($group, $categoryName) {
            return [
