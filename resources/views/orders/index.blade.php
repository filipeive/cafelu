                    ...
                    <table class="table table-hover align-middle">
                        ...
                        <tbody>
                            @forelse ($orders as $order)
                                <tr>
                                    <td class="py-3">
                                        <span class="fw-medium text-primary">#{{ str_pad($order->id, 5, '0', STR_PAD_LEFT) }}</span>
                                    </td>
                                    <td class="py-3">
                                        <div class="d-flex align-items-center">
                                    ...
                                    <td class="py-3">
                                        <div class="badge {{ get_status_class_staradmin2($order->status) }} bg-secondary rounded-pill px-3">
                                    ...
                                    <td class="py-3">
                                        <div class="d-flex justify-content-center gap-2">
                                            <a href="{{ route('orders.show', $order->id) }}" class="btn btn-primary btn-icon btn-sm" data-bs-toggle="tooltip" title="Ver Detalhes">
                                                <i class="mdi mdi-eye"></i>
                                            </a>
                                            @if ($order->status == 'completed' && $order->status != 'canceled')
                                            <a href="{{ route('orders.print', $order->id) }}" class="btn btn-info btn-icon btn-sm" data-bs-toggle="tooltip" title="Imprimir">
                                                <i class="mdi mdi-printer"></i>
                                            </a>
                                            @endif
                                            @if ($order->status != 'completed' && $order->status != 'canceled')
                                                <a href="{{ route('orders.edit', $order->id) }}" class="btn btn-warning btn-icon btn-sm" data-bs-toggle="tooltip" title="Editar">
                                                    <i class="mdi mdi-pencil"></i>
                                                </a>
                                                <button class="btn btn-success btn-icon btn-sm" data-bs-toggle="tooltip" title="Finalizar" onclick="completeOrder({{ $order->id }})">
                                                    <i class="mdi mdi-check"></i>
                                                </button>
                                                <button class="btn btn-danger btn-icon btn-sm" data-bs-toggle="tooltip" title="Cancelar" onclick="cancelOrder({{ $order->id }})">
...
<script>
    document.getElementById('ordersSearch').addEventListener('input', function(e) {
        ...
        this.searchTimeout = setTimeout(() => {
            window.location.href = '{{ route('orders.index') }}?search=' + encodeURIComponent(searchTerm);
        }, 500);
    });
